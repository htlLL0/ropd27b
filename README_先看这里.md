...
本地完整文件夹可复制到另一台机器，`model/` 内含真实基础模型权重（约 55.6 GB），不是下载占位文件、缓存软链接或 LoRA。GitHub 仓库提供除 18 个 `.safetensors` 权重分片外的全部运行包文件；从 GitHub 克隆时，只需按 [README](README.md) 补入同一 revision 的权重。数据、提示、分词器、解析代码和固定配置均已包含。
...
```bash
# 首次在目标机器上下载固定推理镜像；从 GitHub 克隆时先补齐权重。
./docker_b200.sh pull
...
| 路径 | 内容 |
|---|---|
| `model/` | 完整包有 18 个权重分片及 14 个配套文件；GitHub 提供完整 tokenizer/config/license/索引等配套文件，权重另行复制 |
